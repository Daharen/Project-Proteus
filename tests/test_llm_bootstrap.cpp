...
TEST(LlmBootstrapTest, OfflineCacheMissIsDeterministicAndNoRowsWritten) {
    const std::filesystem::path db_path = std::filesystem::temp_directory_path() / "proteus_test_llm_offline.db";
...
TEST(LlmBootstrapTest, CacheReplayIsByteIdentical) {
    const std::filesystem::path db_path = std::filesystem::temp_directory_path() / "proteus_test_llm_cache_replay.db";
    std::filesystem::remove(db_path);
...
TEST(LlmBootstrapTest, ImportDeterminismProposalIdsStableAcrossReruns) {
    const std::filesystem::path db_path = std::filesystem::temp_directory_path() / "proteus_test_llm_import_det.db";
    std::filesystem::remove(db_path);
...
TEST(LlmBootstrapTest, ImportAppliesDeterministicStringCaps) {
    const std::filesystem::path db_path = std::filesystem::temp_directory_path() / "proteus_test_llm_caps.db";
    std::filesystem::remove(db_path);
