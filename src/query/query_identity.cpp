...
std::vector<std::int16_t> fingerprint_dense_v1(std::string_view normalized_text) {
    std::vector<std::int32_t> dense(kFingerprintDims, 0);
    std::string padded = "^" + std::string(normalized_text) + "$";
    for (int n = 3; n <= 5; ++n) {
...
std::vector<std::uint8_t> ComputeSemanticFingerprintV1(std::string_view normalized_text) {
    const auto dense = fingerprint_dense_v1(normalized_text);
...
ClusterResolution ResolveOrAdmitClusterId(
    ...
    const auto fingerprint = fingerprint_dense_v1(normalized);
    ...
    if (alias_stmt.step()) {
        return ClusterResolution{.cluster_id = alias_stmt.column_text(0), .decision_band = "alias_hit", .score = 1.0, .normalized = normalized, .query_id = GetOrCreateQueryId(db, raw_text, query_domain)};
    }
    ...
    if (candidates.size() > static_cast<std::size_t>(kCandidateLimit)) {
    ...
    if (!candidates.empty() && candidates.front().score >= kHardDuplicateThreshold) {
        ...
        ins_alias.bind_text(3, candidates.front().cluster_id);
        ins_alias.step();
        return ClusterResolution{.cluster_id = candidates.front().cluster_id, .decision_band = "hard_duplicate", .score = candidates.front().score, .normalized = normalized, .query_id = query_id};
    }
    ...
    if (!candidates.empty() && candidates.front().score >= kGreyBandThreshold) {
        ...
        if (jaccard >= 0.70 || edit_similarity >= 0.86) {
            ...
            ins_alias.bind_int64(1, static_cast<std::int64_t>(query_domain));
            ins_alias.bind_text(2, normalized);
            ins_alias.bind_text(3, best.cluster_id);
            ins_alias.step();
            ...
            return ClusterResolution{.cluster_id = best.cluster_id, .decision_band = "grey_duplicate", .score = best.score, .normalized = normalized, .query_id = query_id};
        }
        ...
        return ClusterResolution{.cluster_id = best.cluster_id, .decision_band = "requires_server_adjudication", .score = best.score, .normalized = normalized, .query_id = query_id};
    }

    const std::string cluster_id = mint_cluster_id(query_domain, normalized, 11, kFingerprintVersion);
    auto ins_cluster = db.prepare(
        "INSERT OR IGNORE INTO concept_cluster(cluster_id, query_domain, canonical_label, fingerprint_version, fingerprint_blob, created_at_utc) "
        "VALUES(?1, ?2, ?3, ?4, ?5, strftime('%Y-%m-%dT%H:%M:%fZ','now'));"
    );
    ...
    return ClusterResolution{.cluster_id = cluster_id, .decision_band = "novel", .score = 0.0, .normalized = normalized, .query_id = query_id};
}
...
std::vector<FacetTypeSearchHit> SearchFacetTypes(
    ...
    const std::string normalized = NormalizeQuery(raw_text);
    const auto qfp = fingerprint_dense_v1(normalized);
    ...
    if (out.size() > static_cast<std::size_t>(safe_limit)) {
